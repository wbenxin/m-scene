...
function getSignature(timestamp, nonce, token, msg_encrypt) {
  var shasum = crypto.createHash('sha1');
  var arr = [token, timestamp, nonce, msg_encrypt].sort();
    ...
    return async (ctx, next) => {
      ...
      if (method === 'GET') {
        ...
        if (encrypted) {
          ...
          // 校验
          valid = query.msg_signature === getSignature(timestamp, nonce, TOKEN, echostr);
        }
        ...
        if (!this.isDebug && !encrypted) {
          // 校验
          if (query.signature !== getSignature(timestamp, nonce, TOKEN, echostr)) {
            ctx.status = 401;
            ctx.body = 'Invalid signature';
            return;
          }
...
module.exports = function (config, isDebug) {
  return new Wework(config, isDebug);
};
