...
const userid_tokens = new Cache({ stdTTL: 7200, checkperiod: 60, useClones: false });
  ...
  middleware() {
    ...
    router.get('/wework/oauth/:appid/:agentid', async (ctx, next) => {
      ...
      if (ctx.query.code) {
        ...
        let units = redirect_uri.split('#');
        units[0] += (units[0].includes('?') ? '&' : '?') + `appid=${ctx.params.appid}&agentid=${ctx.params.agentid}`;
        ctx.response.redirect(units.join('#'));
      } else {
        // 重定向到授权地址
